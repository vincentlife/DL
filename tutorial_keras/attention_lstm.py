...
class AttentionLSTM(LSTM):
    def __init__(self, units, attention_vec, attn_activation='tanh', single_attention_param=False, **kwargs):
    ...
    def build(self, input_shape):
        ...
        if self.single_attention_param:
            ...
            self.U_s = self.recurrent_initializer((self.units, self.units),
                                       name='{}_U_s'.format(self.name))
            self.b_s = K.zeros((self.units,), name='{}_b_s'.format(self.name))
...
class AttentionLSTMWrapper(Wrapper):
    ...
    def build(self, input_shape=None):
        ...
        self.U_a = self.layer.recurrent_initializer((self.layer.units, self.layer.units), name='{}_U_a'.format(self.name))
        self.b_a = K.zeros((self.layer.units,), name='{}_b_a'.format(self.name))

        self.U_m = self.layer.recurrent_initializer((attention_dim, self.layer.units), name='{}_U_m'.format(self.name))
        self.b_m = K.zeros((self.layer.units,), name='{}_b_m'.format(self.name))

        if self.single_attention_param:
            self.U_s = self.layer.recurrent_initializer((self.layer.units, 1), name='{}_U_s'.format(self.name))
            self.b_s = K.zeros((1,), name='{}_b_s'.format(self.name))
        else:
            self.U_s = self.layer.recurrent_initializer((self.layer.units, self.layer.units), name='{}_U_s'.format(self.name))
